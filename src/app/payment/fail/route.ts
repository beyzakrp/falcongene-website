...
export async function POST(request: NextRequest) {
  try {
    ...
    // Fail sayfasına parametrelerle redirect et
    const redirectUrl = `/payment/fail-result?${params.toString()}`;
    
    // 302 redirect response döndür
    return NextResponse.redirect(new URL(redirectUrl, request.url), 302);
    
  } catch (error) {
    console.error('Payment fail POST callback error:', error);
    
    // Hata durumunda genel fail sayfasına yönlendir
...
export async function GET(request: NextRequest) {
  try {
    ...
    const redirectUrl = `/payment/fail-result?${searchParams.toString()}`;
    
    return NextResponse.redirect(new URL(redirectUrl, request.url), 302);
  } catch (error) {
    console.error('Payment fail GET callback error:', error);
    const failUrl = new URL('/payment/fail-result?error=callback_error', request.url);
    return NextResponse.redirect(failUrl, 302);
  }
}